...
export class NotificationService {
  ...
  static async sendNotification(payload: {
    ...
    if (isLimited) {
      throw new Error(
        "Action blocked: User has reached the daily notification limit.",
      );
    }

    // 3. Check User Preferences & Quiet Hours (Requirement 4.6.10)
    const inQuietHours = await PreferenceService.isInQuietHours(
      userId,
      template.category,
    );
    if (inQuietHours) {
      console.log(
        `[NotificationService] User ${userId} is in quiet hours. Task deferred.`,
      );
